...
class DataConfig(BaseModel):
    ...
    :param dataset_name: name of the dataset to use.
        This should correspond to a subdirectory in
        `data_dir` that contains the dataset files.
    :param data_dir: directory where the dataset
...
class DataModuleConfig(BaseModel):
    ...
    :param data: dataset configuration as in `DataConfig`.
    :param dataloader: dataloader configuration as in `DataloaderConfig`.
    """
...
class CPCDataset(Dataset):
    ...
    def __init__(self, dataset_loaded: dict):
        """Initialize dataset tensors.

        :param dataset_loaded: Dictionary containing ``samples`` and ``labels``
            arrays/tensors. Samples are converted to ``float32`` and labels
            to ``int64``.
        """
        super().__init__()

        samples = dataset_loaded["samples"]
        labels = dataset_loaded["labels"]
        ...
        if samples.ndim == 2:
            logger.debug(
                "Adding channel dimension to samples with shape %s", samples.shape
            )
...
def _stratified_split(
    ...
    :param val_fraction: Fraction of samples to use for validation.
    :param random_state: Random seed for reproducibility.
    :returns: Tuple of train and validation indices.
    """
...
class CPCDataModule(L.LightningDataModule):
    """
    PyTorch Lightning DataModule for CPC training.
    ...
    def setup(self, stage=None):
        ...
        dataset_path = Path(self.config.data.data_dir) / self.config.data.dataset_name

        train_data = torch.load(dataset_path / "train.pt")
        test_data = torch.load(dataset_path / "test.pt")

        full_train_ds = CPCDataset(train_data)
        test_ds = CPCDataset(test_data)
...
class CPCEncoder(nn.Module):
    ...
    def __init__(
        ...
        size_hidden: int = 256,
        conv_specs: list[tuple[int, int, int]] = [],
    ):
...
# %%
config = TrainConfig.from_yaml(Path("configs/config.ecg200.yaml"))
